...
export const api = {
  ...
  addHospital: (data) => request('/hospitals', { method: 'POST', body: JSON.stringify(data) }),
  selectHospital: (id) => request(`/hospitals/${id}/select`, { method: 'POST' }),
  getHospitalPatients: (id) => request(`/hospitals/${id}/patients`),
  getDoctors: () => request('/doctors'),
  getEscalation: () => request('/escalation'),
  getRecommendations: (id) => request(`/patients/${id}/recommendations`),
  getTrajectory: (id) => request(`/patients/${id}/trajectory`),
  addNote: (id, text, author) => request(`/patients/${id}/notes`, { method: 'POST', body: JSON.stringify({ text, author }) }),
